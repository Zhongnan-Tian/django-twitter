...
class FriendshipApiTests(TestCase):
    ...
    def test_follow(self):
        ...
        # need login to follow others
        response = self.anonymous_client.post(url)
        self.assertEqual(response.status_code, 403)

        # cannot use method GET
        ...
        # duplicated follow does nothing
        response = self.user2_client.post(url)
        self.assertEqual(response.status_code, 201)
    ...
    def test_unfollow(self):
        ...
        # unfollow a user you haven't followed, do nothing
        before_count = FriendshipService.get_following_count(self.user2.id)
        response = self.user2_client.post(url)
        self.assertEqual(response.status_code, 200)
