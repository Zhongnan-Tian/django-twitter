...
class HBaseModel:
    ...
    def serialize_field(cls, field, value):
        ...
        if isinstance(field, IntegerField):
            # order alphabetically, could be 1 10 2
            # Fix int to be 16 digits, supply 0 if needed.
            value = str(value)
            while len(value) < 16:
                value = '0' + value
        if field.reverse:
    ...
    def save(self):
        ...
        # If row_data is empty，there is no column key values, hbase won't save
        # the row_key, so raise exception
        if len(row_data) == 0:
